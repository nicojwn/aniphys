...
@overload
def save(func_animation: FuncAnimation, file_name: Path | str) -> None:
    """Save an animation to an explicit file path."""


@overload
def save(func_animation: FuncAnimation, file_type: str = "mp4") -> None:
    """Save an animation using an autogenerated `./aniphysXX` file name."""


@overload
def save(
    func_animation: FuncAnimation, file_name: Path | str, file_type: str = "mp4"
) -> None:
    """Save an animation, using `file_type` only when `file_name` has no extension."""
...
def save(
    ...
    if not isinstance(func_animation, FuncAnimation):
        raise TypeError(
            "func_animation must be a matplotlib.animation.FuncAnimation object"
        )

    if file_name is not None and not isinstance(file_name, (Path, str)):
        raise TypeError("file_name must be a str, pathlib.Path, or None")
    ...
    if file_name is not None:
        if isinstance(file_name, str) and not file_name.strip():
            errors["file_name"] = "must not be empty"
        else:
            file_path = Path(file_name)
            if file_path.exists() and file_path.is_dir():
                errors["file_name"] = "must not be an existing directory"
            elif not file_path.parent.exists():
                errors["file_name"] = "parent directory must exist"
            else:
                suffix = file_path.suffix
                if suffix:
                    if not _is_valid_extension(suffix):
                        errors["file_name"] = "must have a valid extension format"
                else:
                    resolved_file_type = _normalize_file_type(file_type)
                    if not resolved_file_type:
                        errors["file_type"] = (
                            "must not be empty when file_name has no extension"
                        )
                    else:
                        file_path = file_path.with_suffix(f".{resolved_file_type}")
    else:
        resolved_file_type = _normalize_file_type(file_type)
        if not resolved_file_type:
            errors["file_type"] = "must not be empty when file_name has no extension"
            file_path = Path("./aniphys00")
        else:
            file_path = _default_animation_file_name(resolved_file_type)

    _raise_validation_errors(errors)
    func_animation.save(str(file_path))


def _default_animation_file_name(file_type: str) -> Path:
    suffix = 0

    while True:
        candidate = Path(f"./aniphys{suffix:02d}.{file_type}")
...
def _normalize_file_type(file_type: str) -> str:
    stripped_file_type = file_type.strip()
    if not stripped_file_type:
        return ""

    normalized_file_type = stripped_file_type.removeprefix(".")
    if _is_valid_extension(f".{normalized_file_type}"):
        return normalized_file_type

    raise ValueError("file_type must have a valid extension format")


def _is_valid_extension(extension: str) -> bool:
    if not extension.startswith("."):
        return False

    extension_body = extension[1:]
    return extension_body.replace("_", "").isalnum() and bool(extension_body)
